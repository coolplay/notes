...
Fortran 主要用于科学计算，其命名保持简短清晰。这一点不同与其他通用编程语言，命名
往往更加地长。

1. 关键词全部小写
2. 数学变量和函数用相应的数学简称表示
3. 其余的都用小写字母表示：尽量使用单音节或双音节表示，不够清晰的话可使用下划线
...
只使用 module 和 program。用以下方式建立模块(module)::
...
注意上面 ``use`` 语句中模块的显示加载，你也可以通过以下方式进行隐式加载，但最好
少用::
