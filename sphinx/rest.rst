...
* 不可嵌套
* 标记内容不能以空格开头或结束，如 ``* text*`` 有问题。
* 必须用分隔符（如空格）把标记与周围文本分开，词内的标记可以用转义的空格作为分隔
  符。
...
无序列表以星号开始，并且保证段落恰当的缩进；有序列表则在段落开头标上相应的数字，
不过你也可以用 ``#`` 来自动编号::
...
文本区块以 ``::`` 标志开始，并且必须缩进。这一区块的文字将不做任何特殊处理::
...
* 如果某一段落仅仅包含双冒号，这一段落将不会输出。
* 如果双冒号前为空格，双冒号将被移除
* 如果双冒号之前为非空字符，双冒号将变为单冒号。
