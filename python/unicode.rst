...
Unicode起初使用十六位二进制数来表示字符，而非通常的八位。十六位意味着你总共可用
2^16 = 65,536 个不同值，可以涵盖大多数字母表中不同字符；起初Unicode的目标是包含
人类语言中所有的字符表。结果发现即使十六位也无法实现这个目标，更新的Unicode标准
使用了更大范围的代码点，0-1,114,111 (即十六进制表示的0x10ffff)。
...
字符是构成文本的最小结构单元。‘A’，‘B’，‘C’，以及‘我’，‘好’都是不同的字符单元。
...
Unicode标准规定了如何用代码点来表示每一个字符。其中的代码点就是一个整数值，通常
用十六进制表示，比如U+12ca表示代码点为0x12ca（即4810）所对应的字符。Unicode标准
包含了许多字符与代码点对应的表格::
...
严格来说，上面的定义暗含了‘这是字符U+12ca’这种说法是无意义的。U+12ca是一个代码点
，用来表示字符‘ETHIOPIC SYLLABLE WI’。在非正规场合下，代码点与字符间的区分可以忽
略。
...
在屏幕和纸张中，字符用一系列的图像元素表示，称为 **字形** 。字符‘A’的字形用两条
对角线和一条水平线构成，尽管具体的细节与所用的字体密切相关。绝大多数的Python代码
并不需要担心字形问题，这通常是GUI工具包或终端字体渲染器的工作。
...
Python语言内置一个 :class:`unicde` 类型，用以表示Unicode字符串。Unicode类型继承
自一个抽象类型 :class:`basestring` ，这同时也是 :class:`str` 的父类；由此你可以
通过 ``isinstance(value, basestring)`` 来检验对象是否为字符串类型。取决于Python
解释器的编译情况，Unicode字符串会使用16位或32位的整数来表示。
...
:func:`unicode` 构造函数使用方法为 ``unicode（string[, encoding, errors])`` ，其
中所有的参数都必须是8位字符串。第一个参数通过指定的编码转换为unicode，默认使用
ASCII 码，因此对于整数代表值大于127的字符会报错::
